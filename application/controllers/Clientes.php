...
class Clientes extends CI_Controller
{
    ...
    public function index()
    {
        $data = [
            ...
            'clientes' => $this->CoreModel->getAll('clientes'),
        ];
    ...
    {
        if (!$cliente_id || !$this->CoreModel->getById('clientes', ['cliente_id' => $cliente_id])) {
            ...
            if ($this->form_validation->run()) {
                ...
                if ($this->db->table_exists('contas_receber')) {
                    if ($cliente_ativo == 0 && $this->CoreModel->getById('contas_receber', ['conta_receber_cliente_id' => $cliente_id, 'conta_receber_status' => 0])) {
                        $this->session->set_flashdata('info', 'Este cliente não pode ser desativado, pois está sendo usado em <i class="fas fa-hand-holding-usd"></i> contas a receber');
                        redirect('clientes');
                    }
                }
                ...
                $this->CoreModel->update('clientes', $data, ['cliente_id' => $cliente_id]);
                ...
                $data = [
                    ...
                    'cliente' => $this->CoreModel->getById('clientes', ["cliente_id" => $cliente_id])
                ];
    ...
    public function check_rg_ie($cliente_rg_ie)
    {
        ...
        if ($this->CoreModel->getById('clientes', [
    ...
    public function add()
    {
        ...
        if ($this->form_validation->run()) {
            ...
            $this->CoreModel->insert('clientes', $data);
    ...
    {
        if (!$cliente_id || !$this->CoreModel->getById('clientes', ['cliente_id' => $cliente_id])) {
            $this->sesssion->session_flashdata('error', 'Cliente não encontrado');
            redirect('clientes');
        } else {
            $this->CoreModel->delete('clientes', ['cliente_id' == $cliente_id]);
            redirect('clientes');
        }
    }
}
