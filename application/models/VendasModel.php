...
class VendasModel extends CI_Model
{
    public function getAll()
    {
        ...
        $this->db->join('clientes', 'cliente_id = vendas.venda_cliente_id', 'LEFT');
        $this->db->join('vendedores', 'vendedor_id = vendas.venda_vendedor_id', 'LEFT');
        $this->db->join('formas_pagamentos', 'forma_pagamento_id = vendas.venda_forma_pagamento_id', 'LEFT');
        return $this->db->get('vendas')->result();
    }
    public function getAllProdutosByVenda($venda_id = NULL)
    {
        if ($venda_id) {
            $this->db->select([
                'venda_produtos.*',
            ...
            $this->db->join('produtos', 'produto_id = venda_produtos.venda_produto_id_produto', 'LEFT');
    ...
    public function deleteOldProdutos($venda_id = NULL)
    {
        if ($venda_id) {
            $this->db->delete('venda_produtos', [
                'venda_produto_id_venda' => $venda_id
            ]);
        }
    }

    public function getById($venda_id = NULL)
    {
        $this->db->select([
            'clientes.cliente_id',
            'CONCAT(clientes.cliente_nome," ",clientes.cliente_sobrenome) as cliente_nome_completo',
    ...
    public function getAllProdutos($venda_id = NULL)
    {
        if ($venda_id) {
            $this->db->select([
                'venda_produtos.*',
    ...
    // public function getValorFinalVenda($venda_id = NULL)
    // {
    //     if ($venda_id) {
    //         $this->db->select([
    //             'FORMAT(SUM(REPLACE(venda_produto_valor_total,",","")),2) as venda_valor_total',
    ...
    // public function getValorFinalVenda($data_inicial = NULL, $data_final = NULL)
    // {
    //     $this->db->select([
    //         'FORMAT(SUM(REPLACE(venda_valor_total,",","")),2) as venda_valor_total',
    //     ]);
    //     // $this->db->join('vendas', 'venda_id = venda_produtos.venda_produto_id_venda', '', 'LEFT');
